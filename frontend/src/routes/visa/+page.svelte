	...
	<section class="space-y-4 rounded-2xl bg-white p-5 shadow">
		...
		<div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
			...
			<label class="block text-sm font-medium"
				...
				<input
					...
					type="text"
					bind:value={tempVisaData.from_country}
					placeholder="e.g. USA"
				/>
			</label>
			<label class="block text-sm font-medium"
				...
				<input
					class="mt-1 w-full rounded-xl border p-2"
					type="text"
					bind:value={tempVisaData.to_country}
			...
			<label class="block text-sm font-medium"
				...
				<input
					class="mt-1 w-full rounded-xl border p-2"
					type="date"
					bind:value={tempVisaData.from_date}
				/>
			</label>
			<label class="block text-sm font-medium"
				>To Date
				<input
					class="mt-1 w-full rounded-xl border p-2"
					type="date"
					bind:value={tempVisaData.to_date}
				/>
			</label>
		</div>
		...
		<div class="mt-4 flex gap-3">
			<button
				...
				onclick={addVisa}
				disabled={!tempVisaData.service_type ||
					!tempVisaData.from_date ||
					!tempVisaData.to_date ||
					!tempVisaData.from_country ||
					!tempVisaData.to_country}
		...
		<div class="flex items-center gap-3">
			...
			<span class="text-sm text-gray-500"
				...
				(!tempVisaData.from_country ||
					...
					!tempVisaData.from_date ||
					!tempVisaData.to_date)
					? 'Fill out the form or add visa requests'
					: ''}</span
			>
		</div>
